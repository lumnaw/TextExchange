        ...
        while($row = mysqli_fetch_array($result, MYSQLI_NUM))
        {
          ?><img src="nobook.jpg" class="image" alt="Book" style="width:125px; position: absolute; float: left; margin-left: 10px;"><input style="float:right; margin-right: 50px;" type="submit" class="button" value="Add to Cart"><h7><?php
            ...
            {//prints the information of the books(trade:yes)
                printf ("<b>Book Name: %s </b><br> Author: %s %s <br> Subject: %s <br> Edition: %s <br> ISBN: %s <br> Price: %0.2f <br> Seller: %s <br> Description: %s<br> Trade: Yes<br><br>", $row[0], $row[1], $row[2], $row[3], $row[4], $row[5], $row[6], $row[7], $row[8]);
                ?></h7><hr><?php
            }
            else
            {//trade:no
                printf ("<b>Book Name: %s </b><br> Author: %s %s <br> Subject: %s <br> Edition: %s <br> ISBN: %s <br> Price: %0.2f <br> Seller: %s <br> Description: %s<br> Trade: No<br><br>", $row[0], $row[1], $row[2], $row[3], $row[4], $row[5], $row[6], $row[7], $row[8]);
                ?></h7><hr><?php
            }
