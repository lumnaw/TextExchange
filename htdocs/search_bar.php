...
if(($result=$conn->query($query)) == TRUE)
{
    if(mysqli_num_rows($result) > 0)
    {
        while($product = mysqli_fetch_object($result))
        {
            ...
            if(($product->Trade == 1) && ($user != $product->SellerID))//this checks if the book is up for trade or not
            {//prints the information of the books(trade:yes)?> <img src="nobook.jpg" class="image" alt="Book" style="width:125px; position: absolute; float: left; margin-left: 10px;">
